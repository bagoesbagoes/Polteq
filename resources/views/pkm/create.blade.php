<x-layout>
    ...
    <div class="mx-auto max-w-4xl px-4 py-6">
        ...
        <form method="POST" action="{{ route('pkm.store') }}" enctype="multipart/form-data" 
              class="bg-gray-800 shadow sm:rounded-lg p-6" x-data="{ anggotaCount: 0 }">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="border-b border-gray-700 pb-4">
                    <h2 class="text-xl font-semibold text-white">Buat Usulan PKM Baru</h2>
                    <p class="mt-1 text-sm text-gray-400">Isi form di bawah untuk membuat usulan PKM</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="tahun_pelaksanaan" id="tahun_pelaksanaan" 
                               value="{{ old('tahun_pelaksanaan', date('Y')) }}" min="2020" max="2030" required
                               class="mt-2 block w-full rounded-md border-0 bg-gray-700 px-3 py-2 text-white shadow-sm ring-1 ring-inset ring-gray-600 focus:ring-2 focus:ring-indigo-500 sm:text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="sumber_dana" id="sumber_dana" required
                            ...
                            <option value="DIPA" {{ old('sumber_dana') == 'DIPA' ? 'selected' : '' }}>DIPA</option>
                            <option value="Non-DIPA" {{ old('sumber_dana') == 'Non-DIPA' ? 'selected' : '' }}>Non-DIPA</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="kategori_pkm" id="kategori_pkm" required
                            ...
                            <option value="PKM-Karya Cipta" {{ old('kategori_pkm') == 'PKM-Karya Cipta' ? 'selected' : '' }}>PKM-Karya Cipta</option>
                            <option value="PKM-Pengabdian Masyarakat" {{ old('kategori_pkm') == 'PKM-Pengabdian Masyarakat' ? 'selected' : '' }}>PKM-Pengabdian Masyarakat</option>
                            <option value="PKM-Kewirausahaan" {{ old('kategori_pkm') == 'PKM-Kewirausahaan' ? 'selected' : '' }}>PKM-Kewirausahaan</option>
                            <option value="PKM-Karsa" {{ old('kategori_pkm') == 'PKM-Karsa' ? 'selected' : '' }}>PKM-Karsa</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="kelompok_riset" id="kelompok_riset" value="{{ old('kelompok_riset') }}"
                               class="mt-2 block w-full rounded-md border-0 bg-gray-700 px-3 py-2 text-white shadow-sm ring-1 ring-inset ring-gray-600 focus:ring-2 focus:ring-indigo-500 sm:text-sm">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <template x-for="i in anggotaCount" :key="i">
                        <div class="flex gap-2 mt-2">
                            <input type="text" :name="'anggota_tim[' + (i-1) + ']'" placeholder="Nama Anggota"
                ...
                <div>
                    <label for="file_usulan" class="block text-sm font-medium text-white">
                        Upload File Usulan <span class="text-red-500">*</span>
                    </label>
                    <input type="file" name="file_usulan" id="file_usulan" accept=".pdf" required
