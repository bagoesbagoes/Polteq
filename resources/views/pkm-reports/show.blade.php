<x-layout>
    ...
    <div class="mx-auto max-w-5xl px-4 py-6">
            ...
            @if(Auth::user()->role === 'admin')
                ...
                <a href="{{ $type === 'laporan_akhir' ? route('admin.pkm-reports.laporan-akhir') : route('admin.pkm-reports.luaran') }}" 
                   class="inline-flex items-center text-sm text-indigo-400 hover:text-indigo-300">
                    ...
                    Kembali ke Daftar {{ $type === 'laporan_akhir' ? 'Laporan Akhir PKM' : 'Luaran PKM' }}
                </a>
            @else
                {{-- Publisher kembali ke halaman publisher --}}
                <a href="{{ $type === 'laporan_akhir' ? route('pkm-reports.laporan-akhir') : route('pkm-reports.luaran') }}" 
                   class="inline-flex items-center text-sm text-indigo-400 hover:text-indigo-300">
                    <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
        ...
        <div class="bg-gray-800 shadow-lg rounded-lg overflow-hidden border border-gray-700">
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-{{ $type === 'laporan_akhir' ? 'indigo' : 'green' }}-600 text-white mb-3">
                            {{ $type === 'laporan_akhir' ? '📄 Laporan Akhir PKM' : '🎯 Luaran PKM' }}
                        </span>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="bg-gray-900/50 rounded-lg p-5 border border-gray-700">
                    ...
                    @if($report->pkmProposal)
                        <p class="text-white font-medium">{{ $report->pkmProposal->judul }}</p>
                        <p class="text-gray-400 text-sm mt-1">ID PKM: #{{ $report->pkmProposal->id }}</p>
                        <p class="text-gray-400 text-sm">Tahun: {{ $report->pkmProposal->tahun_pelaksanaan }}</p>
                    @else
                        <p class="text-gray-500 italic">Tidak terkait dengan PKM tertentu</p>
                        <p class="text-gray-400 text-sm mt-1">Luaran ini diupload secara mandiri tanpa PKM</p>
                    @endif
                </div>
                ...
                {{-- Jenis Luaran (only for luaran type) --}}
                @if($type === 'luaran' && $report->jenis_luaran)
                    <div class="bg-gray-900/50 rounded-lg p-5 border border-gray-700">
                        <h3 class="text-sm font-semibold text-gray-300 mb-3 flex items-center">
                        ...
                        <p class="text-white font-medium">{{ $report->jenis_luaran }}</p>
                        @if($report->jenis_luaran === 'Lainnya, sebutkan' && $report->jenis_luaran_lainnya)
                            <p class="text-gray-400 text-sm mt-1">Spesifikasi: {{ $report->jenis_luaran_lainnya }}</p>
                        @endif
                    </div>
                @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {{-- Hyperlink --}}
                    @if($report->hyperlink)
                        <div class="bg-gray-900/50 rounded-lg p-5 border border-gray-700 {{ $type === 'luaran' && !$report->file_path ? '' : 'md:col-span-2' }}">
            ...
            {{-- Footer Actions --}}
            <div class="bg-gray-900/50 px-6 py-4 border-t border-gray-700 flex justify-between items-center flex-wrap gap-4">
                
                {{-- Download Surat Tugas (laporan_akhir only) --}}
                @if($report->surat_tugas_path)
                    <div class="flex items-center justify-between p-4 bg-gray-700/50 rounded-lg gap-3 flex-1">
                        <div>
                            <p class="text-white font-medium">Surat Tugas PKM</p>
                        </div>
                        <a href="{{ route('pkm-reports.download-surat-tugas', ['type' => $type, 'report' => $report]) }}" 
                ...
                @if($report->file_path)
                    <div class="flex items-center justify-between p-4 bg-gray-700/50 rounded-lg gap-3 flex-1">
                        <div>
                            <p class="text-white font-medium">File {{ $type === 'laporan_akhir' ? 'Laporan Akhir PKM' : 'Luaran PKM' }}</p>
                        </div>
                ...
                @elseif($report->hyperlink && $type === 'luaran')
                    <a href="{{ $report->hyperlink }}" 
                       target="_blank"
                ...
                <form method="POST" 
                      ...
                      onsubmit="return confirm('⚠️ Yakin ingin menghapus {{ $type === 'laporan_akhir' ? 'laporan akhir PKM' : 'luaran PKM' }} ini? Tindakan ini tidak dapat dibatalkan!')">
                    @csrf
                    @method('DELETE')
