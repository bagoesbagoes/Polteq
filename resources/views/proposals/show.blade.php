                        ...
                        @auth
                            ...
                            @if(Auth::user()->role === 'admin' && $proposal->status !== 'draft')
                                <form method="POST" 
                                      ...
                                      class="inline-block"
                                      onsubmit="return confirm('⚠️ PERHATIAN!\n\nAnda akan menghapus usulan:\n\"{{ $proposal->judul }}\"\n\nStatus: {{ ucfirst(str_replace('_', ' ', $proposal->status)) }}\n\nData yang dihapus TIDAK DAPAT dikembalikan!\n\nLanjutkan menghapus?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                                        <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
